...
export async function POST(request: Request) {
  ...
  try {
    ...
    tempDir = join(tmpdir(), `suno-audio-${crypto.randomUUID()}`);
...
function parseAnalysisError(error: unknown) {
  ...
  if (isExecError(error)) {
    ...
    if (error.killed) return "音声解析がタイムアウトしました。短い音声で試してください。";
  }

  return error instanceof Error ? error.message : "音声解析に失敗しました。";
}
